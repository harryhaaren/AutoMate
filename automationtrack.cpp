...
AutomationTrack::AutomationTrack()
{
  ...
  for (int i = 0; i < 16; i++)
    channel.append_text( floatToStr(176 + i) );
...
{
  std::string string = channel.get_entry()->get_text();

  if (string != "")
  {
    midiChannel = strToInt(string);
    std::cout << "Channel/Command changed: " << midiChannel << std::endl;
  }
...
{
  std::string  string = cc.get_entry()->get_text();
